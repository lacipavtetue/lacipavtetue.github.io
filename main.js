            	...
            	<tr id="`+ i + `" style="font-size: 12px; background-color: ` + bg + ` !important;"> 
            		...
            		<td scope="row" colspan="8">
            			<div class="summary" style="visibility: hidden">
            				...
            				Vos cotisations CIPAV définitives calculées sur vos revenus `+ i + ` sont de <span class="complet"></span> €. Vous devrez donc payer <span class="reste">XXX</span> € (<span class="complet">XXX</span> - <span class="paid">XXX</span>) de régularisation en ` + (i + 1) + ` </b>
            			</div>	
            		</td>
            	</tr>
            `;
	...
	$(".inputrevenu, .inputpaid, .regulpaid, .invalidcheck").on("change ", function() {
		...
		*************************************************/
		...
		if (revenu < classeCotisationsBase[0]) {  //revenus supérieurs au seuil
			...
			row.find(".baseinfo").attr("title", "Vos revenus sont inférieurs à "+classeCotisationsBase[0]+", votre cotisation est basé sur un forfait de "+formatNumber(classeCotisationsBase[1]));
		} else {
			...
			row.find(".baseinfo").attr("title", "<div style='width:400px'><b>Tranche 1:</b><br/>"+formatNumber(tranche1)+" € ("+classeCotisationsBase[3]+" * "+revenu+") <br/><b>Tranche 2:</b><br/>"+formatNumber(tranche2)+" € ("+classeCotisationsBase[5]+" * "+revenu+")</div>");
		}
	  ...
	  *************************************************/
		...
		************************************************************************************************/
		...
		for (i = 0; i < classeRevenus.length; i++) {
			if (revenu <= classeRevenus[i]) {
				...
				row.find(".compleminfo").attr("title", "<div style='width:400px'>Vos revenus sont inférieurs à "+formatNumber(classeRevenus[i])+" €.<br/>Votre classe de cotisation est "+classe+" dont le montant de la cotisation forfaitaire est de "+formatNumber(classeCotisations[i])+" €</div>");
				break;
			}
		}
	   ...
	   *************************************************/
		...
		$("#" + (year + 1)).find(".reste").each(function(i, obj) {
			$(this).text(formatNumber(complet - paid));
		});
		$("#" + (year + 1)).data('regularisation', formatNumber(complet - paid));
	...
	initTooltips();
            		...
            		<td>
            			<div class="input-group mb-3 revenu input-group-sm mb-3" style="width:300px;padding: 0 0 0 0; margin: 0 0 0 0 !important;">
            				...
            				<span style="font-size: 12px;">Les cotisations appelées par la CIPAV en `+ i + ` sont constituées d'un provisionnel (basé sur vos revenus ` + (i - 1) + `) auquels est rajouté la régularisation N-1  
            				<a data-toggle="tooltip" title="<img src='rev.png' />">
            					<i class="bi bi-info-circle" ></i>
            		        </a></span>
            			</div>
            		</td>
	...
	**********************************************************************/
	$(".inputrevenu, .inputpaid, .regulpaid, .invalidcheck").on("change ", function() {
		...
		*************************************************/
		...
		if (revenu < classeCotisationsBase[0]) {
			cotisation_base = classeCotisationsBase[1];
		} else {
			tranche1 = classeCotisationsBase[3] * revenu;
			if (revenu > classeCotisationsBase[2])
